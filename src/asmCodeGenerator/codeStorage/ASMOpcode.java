...
//the machine described here consists of a stack, a separate memory array,
//and a separate instruction store.  There is also a PC (program counter) that
//holds the address (in the instruction store) of the next instruction to execute.

//The stack (Of, say, 2048 locations) is used for most operations.
//Each stack location holds either an integer or a floating-point value.
//The content of memory location l is denoted MEM(l);
//the memory is of some large size and l must be a non-negative int.
//Each memory unit is one byte long.  Floats and integers are stored in 8 and 4 memory
//units, respectively.   IMEM(l...l+3) means the four memory locations l, l+1, l+2, l+3 treated
//as an integer.  FMEM(l...l+7) is similar for a floating-point number.



public enum ASMOpcode {
	// For the following arithmetic instructions, the one or two operands involved 
	// (top element(s) of stack) must be integer.
	// If not, the machine halts.  The result is an int.

	Add,			// adds top two elements of stack, leaving result on stack. [... a b] -> [... a+b]
	Subtract,		// subtracts top of stack from 2nd element.  [... a b] -> [... a-b]
	Negate,			// [... a]   -> [... -a]
	Multiply,		// [... a b] -> [... a*b]
	Divide,			// [... a b] -> [... a/b]
	...
	// Type conversions.
	ConvertF,		// Convert the top of the stack from int to floating. Halt if top isn't int.
	ConvertI,		// Convert the top of the stack from floating to int. Halt if top isn't floating.
					// The conversion is a truncation towards zero (floor for positives, ceiling for negatives).

	// Stack manipulation
	Duplicate,		// [... a] -> [... a a]		(duplicate the top element of the stack)
	Exchange,		// [... a b] -> [... b a]
	...
	LoadC,			// top of stack is treated as a location, and replaced with the contents of
					// that memory location: [... a] -> [... MEM(a)]	(MEM(a) is of type integer)
	LoadI,			// [... a] -> [... IMEM(a..a+3)]
	LoadF,			// [... a] -> [... FMEM(a..a+7)]
	
	StoreC,			// top of stack, which must be an integer, is ANDed with 0xff 
	...
	Memtop,			// pushes the size s of the memory onto the stack.  This is an invalid address,
	...
	PStack,			// Nondestructively prints a copy of the current ASM stack.  For debugging purposes.

	// The one final and absolutely amazing opcode
	Printf,			// Does a C-style printf, with args taken from the top of the stack
